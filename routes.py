...
# ---------------- Indicator specs (must match utils.compute_indicators) ------
...
def _save_indicators_from_form(cfg: dict, form) -> None:
    ...
    for key, spec in INDICATOR_SPECS.items():
        ...
        for f_name, default in spec.get("fields", {}).items():
            raw = form.get(f"ind_{key}_{f_name}")
            if raw is None or raw == "":
                params[f_name] = ind.get(key, {}).get(f_name, default)
            else:
                try:
                    if isinstance(default, int):   params[f_name] = int(float(raw))
                    elif isinstance(default, float): params[f_name] = float(raw)
                    else: params[f_name] = raw
                except Exception:
...
def api_ind_toggle():
    ...
    set_config(cfg)
    return jsonify({"ok": True, "indicator": {key: cur}})
...
def api_ind_toggle_all():
    ...
    for k, spec in INDICATOR_SPECS.items():
        if panel in ("all", spec["panel"]):
            cur = cfg["indicators"].get(k, {"enabled": False, **spec["fields"]})
            cur["enabled"] = enabled
            cfg["indicators"][k] = cur
    set_config(cfg)
    return jsonify({"ok": True, "panel": panel, "enabled": enabled})
