# routes.py — Dashboard, config, backtest, Telegram + Live APIs (with Users)
...
# ---------- Live engine (keep imports tolerant) ----------
try:
    from live_engine import ENGINE, TIER_TO_CHAT, DAILY_CAPS
except Exception:
    # Minimal stub expectations so dashboard still renders
    class _Stub:
        def status(self): return {"running": False, "debug": False, "tally": {"total":0,"by_tier":{}}}
...
# Create table on import (id, tier, chat_id unique, label)
...
# --------------------------- Helpers -----------------------------------------
def _ensure_cfg_defaults(cfg: dict) -> dict:
    cfg = dict(cfg or {})
    # Window (Mon–Fri enforced by utils.within_window)
    ...
    # Indicators (first run: everything off but fields present)
...
def api_status_dup():
    s = ENGINE.status()
    # Smooth/compact object as requested
    out = {
        "caps": DAILY_CAPS,
        "configured_chats": {k: bool(v) for k, v in (TIER_TO_CHAT or {}).items()},
