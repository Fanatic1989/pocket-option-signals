...
# ===================== Indicator catalog (matches your TV list) =====================
INDICATOR_SPECS: Dict[str, Dict[str, Any]] = {
    ...
    "ALLIGATOR":  {"name":"Alligator (SMA triplet)","kind":"overlay","params":{"jaws":13,"teeth":8,"lips":5}, "enabled":False},
...
def _expiry_bars(exp: str) -> int:
    return {"1m":1,"3m":3,"5m":5,"10m":10,"30m":30,"1h":60,"4h":240}.get((exp or "5m").lower(), 5)
...
def update_symbols():
    ...
    cfg["active_symbols"] = convert_po_to_deriv(tokens) if convert_po else tokens
    _save_cfg(cfg)
...
def backtest():
    ...
    if "bt_csv" in request.files and request.files["bt_csv"].filename:
        ...
        for sym in try_syms or ["frxEURUSD"]:
            try:
                # ---------------------------- FIX HERE: no 'days' kwarg ----------------------------
                df = fetch_deriv_history(sym, gran)  # your utils version doesn't accept 'days'
                # -----------------------------------------------------------------------------------
                break
            except Exception as e:
                ...
                df = None
        if df is None:
            tail = "; ".join(attempts[-3:]) if attempts else "no attempts"
            flash(f"Backtest error: Deriv fetch failed. {tail}", "error")
            return redirect(url_for("dashboard.view"))
    else:
...
def live_tally():
    ...
    return jsonify({"tally": {"free": by.get("free",0), "basic": by.get("basic",0), "pro": by.get("pro",0), "vip": by.get("vip",0), "all": all_total}})
