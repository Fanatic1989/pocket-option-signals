# routes.py — Dashboard, config, backtest, Telegram + Live APIs (+ Indicators toggle-all)
...
from utils import (
    TZ, get_config, set_config, within_window, exec_sql,
...
def _ensure_cfg_defaults(cfg: dict | None) -> dict:
    ...
    cfg.setdefault("custom1_rules",""); cfg.setdefault("custom2_rules",""); cfg.setdefault("custom3_rules","")
...
def api_ind_params():
    ...
    for fname, default in spec["fields"].items():
        if fname in params:
            val = params[fname]
            try:
                if isinstance(default, int): cur[fname] = int(float(val))
                elif isinstance(default, float): cur[fname] = float(val)
                else: cur[fname] = val
            except Exception:
                cur[fname] = default
    cfg["indicators"][key] = cur
...
def backtest():
    ...
    raw = (request.form.get("bt_symbols") or "").replace(";", ",")
    symbols = [s.strip() for s in raw.split(",") if s.strip()] or cfg.get("symbols_raw", [])
    ...
    strat = request.form.get("bt_strategy", "BASE").upper()
    sigs, stats = backtest_run(df, strat, ind_cfg, expiry)
    png_name = plot_signals(df, sigs, ind_cfg, strategy=strat, tf=tf, expiry=expiry)

    return jsonify({
        "ok": True,
        "plot": f"/plots/{png_name}",
        "signals": [{"ts": s["index"].isoformat(), "dir": s["direction"], "exp": s["expiry_idx"].isoformat()} for s in sigs],
        "stats": stats
    })
...
def users_add():
    ...
    try:
        exec_sql("INSERT OR IGNORE INTO tg_users(tier,chat_id,label) VALUES(?,?,?)", (tier, chat_id, label))
        flash("User added.", "ok")
    except Exception as e:
...
def api_status_dup():
    ...
    out = {
        ...
        "tally": s.get("tally", {"date": datetime.now(TZ).strftime("%Y-%m-%d"), "total": 0, "by_tier": {"free":0,"basic":0,"pro":0,"vip":0}}),
    }
    return jsonify(out)
